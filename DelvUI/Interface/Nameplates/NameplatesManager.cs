...
namespace DelvUI.Interface.Nameplates
{
    internal class NameplatesManager : IDisposable
    {
        ...
        public unsafe void Update()
        {
            ...
            if (AetherUiNameplatesExtensionRegistry.OverridePreviewData != null)
            {
                _data = new List<NameplateData>(AetherUiNameplatesExtensionRegistry.OverridePreviewData);
                return;
            }
    ...
    public struct NameplateData
    {
        ...
        public bool PreferDisplayName;

        public NameplateData(IGameObject? gameObject, string name, string title, bool isTitlePrefix, int namePlateIconId, string order, ObjectKind kind, byte subKind, Vector2 screenPosition, Vector3 worldPosition, float distance, bool ignoreOcclusion = false, bool preferDisplayName = false)
        {
