...
namespace DelvUI.Interface.Nameplates
{
    internal class NameplatesHud : HudElement
    {
        ...
        protected override void CreateDrawActions(Vector2 origin)
        {
            ...
            {
                NameplateData drawData = data;
                Nameplate? nameplate = GetNameplate(drawData);
                if (nameplate == null || !nameplate.Enabled) { continue; }

                // raycasting
                if (IsPointObstructed(drawData)) { continue; }

                if (nameplate is NameplateWithBar nameplateWithBar)
                {
                    // draw bar
                    AddDrawActions(nameplateWithBar.GetBarDrawActions(drawData));

                    // find mouseovered nameplate
                    var (isHovering, ignore) = nameplateWithBar.GetMouseoverState(drawData);
                    if (isHovering)
                    {
                        mouseoveredActor = drawData.GameObject;
                        ignoreMouseover = ignore;
                    }
                }

                // draw elements
                AddDrawActions(nameplate.GetElementsDrawActions(drawData));
            }
