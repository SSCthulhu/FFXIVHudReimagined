...
namespace DelvUI.Interface.GeneralElements
{
    public class CastbarHud : ParentAnchoredDraggableHudElement, IHudElementWithActor, IHudElementWithAnchorableParent, IHudElementWithPreview
    {
        ...
        public override void DrawChildren(Vector2 origin)
        {
            ...
            if (Config is PlayerCastbarConfig)
            {
            ...
            if (Config.ShowIcon && validIcon)
            {
                ...
                AddDrawAction(Config.StrataLevel, () =>
                {
                    DrawHelper.DrawInWindow(ID + "_icon", finalIconPos, finalIconSize, false, (drawList) =>
                    {
                        ...
                        {
                            float iconRounding = Math.Clamp(Config.CornerRounding, 0f, Math.Min(finalIconSize.X, finalIconSize.Y) / 2f);
                            drawList.AddRect(finalIconPos, finalIconPos + finalIconSize, Config.BorderColor.Base, iconRounding, ImDrawFlags.None, Config.BorderThickness);
                        }
                    });
                });
            }
        ...
        private unsafe void UpdateCurrentCast(out float currentCastTime, out float totalCastTime)
        {
            ...
            try
            {
                ...
                if (castInfo != null && castInfo->TotalCastTime > 0)
                {
            ...
            if (total <= 0 || (!Utils.IsActorCasting(battleChara) && current <= 0))
            {
                currentCastTime = 0;
                totalCastTime = 0;
                return;
    ...
    public class NameplateCastbarHud : TargetOfTargetCastbarHud
    {
        ...
        {
            // Nameplate castbars should not depend on target/focus APIs.
            // DrawChildren already checks the actor's real cast progress.
            return true;
        }
    }
}
