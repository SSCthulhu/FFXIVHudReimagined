...
namespace DelvUI.Interface.GeneralElements
{
    ...
    public class WindowClippingConfig : PluginConfigObject
    {
        ...
        public bool Draw(ref bool changed)
        {
            ...
            ImGuiHelper.SetTooltip("When enabled, Nameplates will get covered by game UI elements that wouldn't normally cover Aether UI elements.");
            ...
            if (NameplatesClipRectsEnabled)
            {
                ...
                ImGuiHelper.SetTooltip("When enabled, the game's target castbar will not be covered by Aether UI Nameplates.\nFor players that prefer to use the default target cast bar over Aether UI's.");

                ImGuiHelper.Tab(); ImGuiHelper.Tab();
                changed |= ImGui.Checkbox("Hotbars", ref HotbarsClipRectsEnabled);
                ImGuiHelper.SetTooltip("When enabled, active hotbar will not be covered by Aether UI Nameplates.\nNote that the way this is calculated is not perfect and it might not work well for hotbars that have empty slots.");
            ...
            ImGuiHelper.SetTooltip("When enabled, other plugins' windows can also be clipped so Aether UI elements don't cover them.\nPlease note that this requires the developer of each third party plugin to implement the feature.");
            ...
            switch (Mode)
            {
                case WindowClippingMode.Full:
                    ImGui.Text("Aether UI will attempt to not cover game windows in this mode by clipping around them.");
                    break;

                case WindowClippingMode.Hide:
                    ImGui.Text("Aether UI will attempt to not cover game windows in this mode by not drawing an element if it's touching a game window.");
                    break;
