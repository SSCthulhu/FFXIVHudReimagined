...
namespace DelvUI.Helpers
{
    ...
    public static class DrawHelper
    {
        ...
        public static void DrawBarTexture(Vector2 position, Vector2 size, PluginConfigColor color, string? name, BarTextureDrawMode drawMode, ImDrawListPtr drawList, float rounding = 0f)
        {
            IDalamudTextureWrap? texture = BarTexturesManager.Instance?.GetBarTexture(name);
            if (texture == null)
            {
                DrawGradientFilledRect(position, size, color, drawList, ConfigurationManager.Instance.GradientDirection, rounding);
                return;
            }
            ...
            drawList.AddImageRounded(texture.Handle, position, position + size, uv0, uv1, color.Base, rounding);
            drawList.PopClipRect();
        }

        public static void DrawGradientFilledRect(Vector2 position, Vector2 size, PluginConfigColor color, ImDrawListPtr drawList, float rounding = 0f)
        {
            GradientDirection gradientDirection = ConfigurationManager.Instance.GradientDirection;
            DrawGradientFilledRect(position, size, color, drawList, gradientDirection, rounding);
        }

        public static void DrawGradientFilledRect(Vector2 position, Vector2 size, PluginConfigColor color, ImDrawListPtr drawList, GradientDirection gradientDirection = GradientDirection.Down, float rounding = 0f)
        {
            uint[]? colorArray = ColorArray(color, gradientDirection);
            bool rounded = rounding > 0f;
            ...
            if (gradientDirection == GradientDirection.CenteredHorizonal)
            {
                ...
                if (rounded)
                {
                    // Rounded centered gradients are approximated with solid fill in this path.
                    drawList.AddRectFilled(position, position + size, color.Base, rounding);
                }
                else
                {
            ...
            else
            {
                if (rounded)
                {
                    // Rounded gradients are approximated with solid fill in this path.
                    drawList.AddRectFilled(position, position + size, color.Base, rounding);
                }
                else
                {
        ...
        public static void DrawOvershield(float shield, Vector2 cursorPos, Vector2 barSize, float height, bool useRatioForHeight, PluginConfigColor color, ImDrawListPtr drawList)
        {
            ...
            DrawGradientFilledRect(cursorPos, new Vector2(Math.Max(1, barSize.X * shield), h), color, drawList, 0f);
        }
        ...
        public static void DrawShield(float shield, float hp, Vector2 cursorPos, Vector2 barSize, float height, bool useRatioForHeight, PluginConfigColor color, ImDrawListPtr drawList)
        {
            ...
            DrawGradientFilledRect(shieldStartPos, new Vector2(Math.Max(1, barSize.X * s), barSize.Y), color, drawList, 0f);
