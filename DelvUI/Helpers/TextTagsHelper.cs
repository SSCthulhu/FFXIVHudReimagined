...
namespace DelvUI.Helpers
{
    public static class TextTagsHelper
    {
        ...
        public static Dictionary<string, Func<ICharacter, string>> CharaTextTags = new Dictionary<string, Func<ICharacter, string>>()
        {
            ...
            ["[shield:percent]"] = (chara) =>
            {
                float shield = Utils.ActorShieldValue(chara) * 100f;
                return Math.Clamp(shield, 0f, 100f).ToString("N0", ConfigurationManager.Instance.ActiveCultreInfo);
            },

            ["[shield:amount]"] = (chara) =>
            {
                float shield = Utils.ActorShieldValue(chara);
                uint amount = (uint)MathF.Round(chara.MaxHp * shield);
                return amount.ToString("N0", ConfigurationManager.Instance.ActiveCultreInfo);
            },
            ...
            ["[shield:text]"] = (chara) =>
            {
                ...
                float percent = Math.Clamp(shield * 100f, 0f, 100f);
                uint amount = (uint)MathF.Round(chara.MaxHp * shield);
                return $"{percent.ToString("N0", ConfigurationManager.Instance.ActiveCultreInfo)}% (+{amount.ToString("N0", ConfigurationManager.Instance.ActiveCultreInfo)})";
            }
            #endregion
        };
