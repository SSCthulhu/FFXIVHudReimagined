...
public sealed class HudWindow
{
    ...
    private DateTime minimapControlsVisibleUntil = DateTime.MinValue;

    public HudWindow(HudConfiguration config, HudStateProvider stateProvider, Action? openConfigAction = null)
    {
        this.config = config;
    ...
    public void Draw(bool skipInteractiveOverlay = false)
    {
        ...
        DrawMinimapControlsOverlay(layout);
    ...
    private void DrawMinimapControlsOverlay(HudLayoutRects layout)
    {
        if (!this.config.MinimapEnabled)
        {
            return;
        ...
        var minimapHovered = this.IsPointInMinimapArea(mousePos, layout);
        if (minimapHovered)
        {
            this.minimapControlsVisibleUntil = DateTime.UtcNow + MinimapControlsGrace;
        }

        var controlsVisible = minimapHovered || DateTime.UtcNow < this.minimapControlsVisibleUntil;
        if (!controlsVisible)
        {
            return;
        }

        var size = MinimapLayout.ClampSize(this.config.MinimapSize);
        var half = size * 0.5f;
        ...
        var controlsPos = this.GetMinimapControlsPosition(layout, controlsWidth, controlsHeight);
        ...
        ImGui.Begin("FFXIVHudPlugin##MinimapControls", controlsFlags);
        ...
        {
            this.config.MinimapVisibleRangeYalms = MinimapLayout.ClampVisibleRange(this.config.MinimapVisibleRangeYalms + 8f);
            this.config.Save();
        }
        ...
        {
            this.config.MinimapNorthLocked = !this.config.MinimapNorthLocked;
            this.config.Save();
            northLocked = this.config.MinimapNorthLocked;
        }
        DrawMinimapLockIcon(
    ...
    private bool IsPointInMinimapArea(Vector2 point, HudLayoutRects layout)
    {
        var size = MinimapLayout.ClampSize(this.config.MinimapSize);
        var half = size * 0.5f;
        var center = layout.MinimapCenter;
        if (this.config.MinimapSquare)
        {
    ...
    private static void DrawMinimapCogIcon(ImDrawListPtr draw, Vector2 min, Vector2 max, uint color)
    {
        ...
        for (var i = 0; i < 8; i++)
        {
            ...
            draw.AddLine(center + (dir * toothInner), center + (dir * toothOuter), color, 1.7f);
        }

        draw.AddCircle(center, buttonSize * 0.24f, color, 16, 1.8f);
        draw.AddCircle(center, buttonSize * 0.12f, color, 16, 1.6f);
    }
    ...
    private static void DrawMinimapLockIcon(ImDrawListPtr draw, Vector2 min, Vector2 max, uint color)
    {
        ...
        var bodyMin = new Vector2(center.X - (bodyWidth * 0.5f), center.Y + (buttonSize * 0.05f));
        ...
        var shackleCenter = new Vector2(center.X, bodyMin.Y - (buttonSize * 0.02f));
        draw.PathClear();
        ...
        var keyholeTop = new Vector2(center.X, bodyMin.Y + (bodyHeight * 0.42f));
    ...
    private Vector2 GetMinimapControlsPosition(HudLayoutRects layout, float controlsWidth, float controlsHeight)
    {
        var size = MinimapLayout.ClampSize(this.config.MinimapSize);
        var half = size * 0.5f;
        var center = layout.MinimapCenter;
        ...
        {
            // Bottom-right point on circular border, then align controls so its lower-right corner hugs that arc.
